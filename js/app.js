...
class FreeFallApp {
    ...
    onPlanetChange(planet, suffix = '') {
        ...
        if (planet === 'moon') {
            ...
            this.showTip('달에는 대기가 없어 공기저항이 발생하지 않습니다.', 'air-tip' + suffix);
        } else {
    ...
    onAirResistanceChange(enabled, suffix = '') {
        ...
        if (enabled) {
            tipEl.textContent = '공기 저항이 작용하면 가벼운 물체일수록 더 큰 영향을 받습니다.';
            tipEl.style.display = 'block';
        } else {
            tipEl.textContent = '공기 저항이 없으면 질량에 관계없이 모든 물체가 같은 빠르기로 떨어집니다.';
            tipEl.style.display = 'block';
        }
        this.onParameterChange();
    }
    ...
    updateParameterDisplay() {
        ...
        document.getElementById('est-time').textContent = theoreticalTime.toFixed(2) + ' 초';
    }
    ...
    getInsight(planet, objectType, airRes, state) {
        ...
        if (!airRes) {
            insights.push('공기저항이 없으므로 물체의 질량에 관계없이 같은 시간 동안 같은 거리를 낙하합니다.');
            insights.push(`이론값: 낙하 시간 = √(2 × ${this.physics.initialHeight} ÷ ${this.physics.gravity}) = ${this.physics.getTheoreticalTime().toFixed(2)} 초`);
        } else {
            const tv = this.physics.getTerminalVelocity();
            if (state.velocity >= tv * 0.95) {
                insights.push(`이 물체는 종단속도(약 ${tv.toFixed(1)} m/s)에 거의 도달했습니다. 종단속도란 중력과 공기저항이 같아져서 더 이상 빨라지지 않는 속력입니다.`);
            }
            insights.push('공기저항이 있으면 가볍고 넓은 물체일수록 더 천천히 떨어집니다.');
        }
